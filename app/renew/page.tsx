...
function RenewalPaymentForm({ orderId }: { orderId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: stripeError } = await stripe.confirmPayment({
      elements,
      confirmParams: {
        return_url: `${window.location.origin}/success?order_id=${orderId}&renewed=1`,
      },
    });
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button className="btn-pay" type="submit" disabled={!stripe || paying}>
        {paying ? 'Processing…' : 'Renew for $47 — another 365 mornings'}
      </button>
      <p className="pay-note">One-time payment. No subscription. No recurring charges.</p>
    </form>
  );
}
    ...
    fetch('/api/renew', {
      ...
      .then(json => {
        if (json.clientSecret) {
          ...
          setError(json.error ?? 'Could not set up renewal. Please try again.');
        }
        setLoading(false);
      })
      .catch(() => {
  ...
  if (error || !clientSecret || !orderId) {
    return (
      <div className="renew-error">
        ...
        <Link href="/">← Back to Seraphova</Link>
      </div>
    );
  }
      ...
      <main className="renew-main">
        <div className="renew-wrap">
          ...
          <div className="renew-summary">
            <p className="summary-eyebrow">✦ Seraphova · Renewal</p>
            <h1 className="summary-title">Another year.<br /><em>Same chart, new sky.</em></h1>
            <div className="summary-price">
              <span className="summary-amount">47</span>
              <span className="summary-period">one-time · 365 more mornings</span>
            </div>
            <ul className="summary-features">
              {[
                'Your natal chart stays the same — readings pick up where they left off',
                'New transits every single morning',
                'Same delivery time, same inbox',
          ...
          {/* Payment form */}
          <div className="renew-form-wrap">
            <div className="renew-card">
              <p className="form-label">Complete your renewal</p>
              <Elements
              ...
              >
                <RenewalPaymentForm orderId={orderId} />
              </Elements>
            </div>
          </div>
...
.renew-main { flex:1; display:flex; align-items:flex-start; justify-content:center; padding:60px 24px 80px; }
.renew-wrap { width:100%; max-width:860px; display:grid; grid-template-columns:1fr 1fr; gap:48px; align-items:start; }

.renew-summary { padding-top:8px; }
...
.summary-title em { font-style:italic; color:var(--gold2); }
...
.summary-features li { display:flex; align-items:flex-start; gap:12px; font-size:14px; color:var(--muted); line-height:1.6; }
.summary-check { color:var(--gold); font-size:9px; flex-shrink:0; margin-top:5px; }

.renew-form-wrap { }
.renew-card { background:var(--card); border:1px solid var(--border2); border-radius:var(--r2); padding:32px; box-shadow:0 24px 60px rgba(0,0,0,0.4); }
...
.renew-loading { display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:100vh; gap:24px; }
.renew-loading p { font-size:14px; color:var(--muted); }
.renew-error { display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:100vh; text-align:center; gap:16px; padding:40px; }
.renew-error p { color:var(--muted); font-size:16px; }
.renew-error a { font-size:14px; color:var(--gold2); text-decoration:none; }
...
@media (max-width:720px) {
  .renew-wrap { grid-template-columns:1fr; gap:32px; }
  .renew-main { padding:40px 16px 60px; }
}
`;
